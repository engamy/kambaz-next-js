...
export default function Dashboard() {
  ...
  const onAddNewCourse = async () => {
    ...
    try {
      ...
      // Omit _id from courseData when creating a new course
      const { _id, ...courseData } = course; // eslint-disable-line @typescript-eslint/no-unused-vars
        ...
        <Row xs={1} md={5} className="g-4">
          {courses.map((course) => {
            ...
            const courseName = typeof course.name === 'string' ? course.name : 
                              (typeof course.title === 'string' ? course.title : "Unnamed Course");
            const courseDescription = typeof course.description === 'string' ? course.description : 
                                     (typeof course.desc === 'string' ? course.desc : "No description available");
            return (
